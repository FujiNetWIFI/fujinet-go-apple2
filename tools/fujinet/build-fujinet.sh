#!/usr/bin/env bash
#
# Build the FujiNet APPLE runtime as a shared library (libfujinet.so) plus its
# default runtime assets (fnconfig.ini / data / SD) for the Android app.
#
# Adapted from fujinet-go-adam's tools/fujinet/build-fujinet.sh. Differences:
#   * Source is the user's LOCAL fujinet-pc-apple2 checkout (not a pinned GitHub
#     tarball), staged into work/fujinet-firmware so the checkout is untouched.
#   * The PC build targets APPLE (build.sh -cp APPLE), whose IWM SmartPort bus
#     uses the SmartPort-over-SLIP transport (iwm_slip / connector_net) and
#     connects out to AppleWin's CT_SmartPortOverSlip Listener on TCP 1985.
#   * fnconfig.ini is forced to [BOIP] enabled=1 host=127.0.0.1 port=1985 (the
#     APPLE build reuses the [BOIP] host/port keys as its SLIP endpoint) so the
...
# transforms to the staged copy -- the same fujinet-pc codebase as the ADAM
...
# Local fujinet-pc-apple2 checkout (override with FUJINET_SRC=/path ...).
FUJINET_SRC="${FUJINET_SRC:-${HOME}/Workspace/fujinet-pc-apple2}"
PC_TARGET="APPLE"
...
Builds the FujiNet APPLE Android runtime from the local fujinet-pc-apple2 checkout.
EOF
}
...
stage_local_source() {
    [[ -d "${FUJINET_SRC}" ]] || fail "fujinet-pc-apple2 source not found at ${FUJINET_SRC} (set FUJINET_SRC)"
...
# moves the CWD after startup (AppleWin's chdir), so the load silently fails and
...
force_boip_config() {
    # Ensure the runtime's SmartPort-over-SLIP client connects to AppleWin's
    # in-process CT_SmartPortOverSlip Listener (the APPLE build reads the SLIP
    # endpoint from the [BOIP] host/port keys).
...
echo "FujiNet APPLE Android runtime outputs updated:"
